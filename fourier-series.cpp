...
double fourierk (double *x, double *par) {
	...
	double y = ak * normalize *cos(coeff_cos*x[0]) +
			bk * normalize *sin(coeff_sin*x[0]) ; //x[i]
	return y;
}
...
int main (int numArg, char * listArg[])
{
	...
	int iter = 101; // numero di termini della serie di fourier
	...
	int GraphStep = 1 ; //ogni quante iterazioni stampare il grafico/
...
//				std::cout << i << " " << x[i] << " " << fx[i] <<  std::endl;
				...
				graph->SetTitle(Form(
					"f(x)=#Sigma_{k=%d}^{%d}a_{k}cos(kx)+b_{k}sin(kx) ",
					start,k));
